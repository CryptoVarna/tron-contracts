...
const { expectEvent } = require("@cryptovarna/tron-test-helpers");
const { expect } = require("chai");

const TRC2771ContextMock = artifacts.require("TRC2771ContextMock");
...
contract("TRC2771Context", function (accounts) {
    beforeEach(async function () {
        this.forwarder = await MinimalForwarder.new();
        this.recipient = await TRC2771ContextMock.new(this.forwarder.address);
    ...
    context("when receiving a relayed call", function () {
        ...
        describe("msgSender", function () {
            it("returns the relayed transaction original sender", async function () {
                ...
                await expectEvent.inTransaction(tx, TRC2771ContextMock, "Sender", { sender: this.sender });
            });
        });
        ...
        describe("msgData", function () {
            it("returns the relayed transaction original data", async function () {
                ...
                const { tx } = await this.forwarder.execute(req, sign);
                await expectEvent.inTransaction(tx, TRC2771ContextMock, "Data", { data, integerValue, stringValue });
            });
        });
    });
});
