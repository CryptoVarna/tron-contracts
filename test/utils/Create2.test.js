const { balance, BN, ether, expectRevert, send } = require("@cryptovarna/tron-test-helpers");

const { expect } = require("chai");

const Create2Impl = artifacts.require("Create2Impl");
const TRC20Mock = artifacts.require("TRC20Mock");
const TRC1820Implementer = artifacts.require("TRC1820Implementer");
...
contract("Create2", function (accounts) {
    ...
    const constructorByteCode = `${TRC20Mock.bytecode}${encodedParams}`;
    ...
    describe("deploy", function () {
        it("deploys a TRC1820Implementer from inline assembly code", async function () {
            const offChainComputed =
                computeCreate2Address(saltHex, TRC1820Implementer.bytecode, this.factory.address);
            await this.factory.deployTRC1820Implementer(0, saltHex);
            expect(TRC1820Implementer.bytecode).to.include((await web3.eth.getCode(offChainComputed)).slice(2));
        });

        it("deploys a TRC20Mock with correct balances", async function () {
            ...
            const TRC20 = await TRC20Mock.at(offChainComputed);
            expect(await TRC20.balanceOf(deployerAccount)).to.be.bignumber.equal(new BN(100));
        });
