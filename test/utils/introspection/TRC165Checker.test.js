...
const TRC165CheckerMock = artifacts.require("TRC165CheckerMock");
const TRC165MissingData = artifacts.require("TRC165MissingData");
const TRC165NotSupported = artifacts.require("TRC165NotSupported");
const TRC165InterfacesSupported = artifacts.require("TRC165InterfacesSupported");
...
contract("TRC165Checker", function (accounts) {
    beforeEach(async function () {
        this.mock = await TRC165CheckerMock.new();
    });

    context("TRC165 missing return data", function () {
        beforeEach(async function () {
            this.target = await TRC165MissingData.new();
        });

        it("does not support TRC165", async function () {
            const supported = await this.mock.supportsTRC165(this.target.address);
    ...
    context("TRC165 not supported", function () {
        beforeEach(async function () {
            this.target = await TRC165NotSupported.new();
        });
    ...
    context("TRC165 supported", function () {
        beforeEach(async function () {
            this.target = await TRC165InterfacesSupported.new([]);
        });
    ...
    context("TRC165 and single interface supported", function () {
        beforeEach(async function () {
            this.target = await TRC165InterfacesSupported.new([DUMMY_ID]);
        });
    ...
    context("TRC165 and many interfaces supported", function () {
        beforeEach(async function () {
            this.supportedInterfaces = [DUMMY_ID, DUMMY_ID_2, DUMMY_ID_3];
            this.target = await TRC165InterfacesSupported.new(this.supportedInterfaces);
        });
    ...
    context("account address does not support TRC165", function () {
        it("does not support TRC165", async function () {
            const supported = await this.mock.supportsTRC165(DUMMY_ACCOUNT);
            expect(supported).to.equal(false);
        });
