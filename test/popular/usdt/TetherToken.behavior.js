...
function shouldBehaveLikeTRC20(errorPrefix, initialSupply, initialHolder, recipient, anotherAccount) {
    describe("total supply", function () {
        it("returns the total amount of tokens", async function () {
            expect(BN.fromHex(await this.token.totalSupply().send({ shouldPollResponse: true }))).to.be.bignumber.equal(initialSupply);
        });
    });
                ...
                describe("when the spender has enough approved balance", function () {
                    ...
                    describe("when the token owner has enough balance", function () {
                        ...
                        it("decreases the spender allowance", async function () {
                            ...
                            expect((await this.token.allowance(tokenOwner, spender).call()).remaining).to.be.bignumber.equal("0");
                        });
...
function shouldBehaveLikeTRC20Transfer(errorPrefix, from, to, balance, transfer) {
    describe("when the recipient is not the zero address", function () {
        describe("when the sender does not have enough balance", function () {
            ...
            it("reverts", async function () {
                account.setDefault(from);
                // @todo fix the expected exception
        ...
        describe("when the sender has enough balance", function () {
            ...
            describe("when there was no approved amount before", function () {
                it("approves the requested amount", async function () {
                    ...
                    expect(BN.fromHex((await this.token.allowance(owner, spender).call()).remaining)).to.be.bignumber.equal(amount);
                });
            });
        ...
        describe("when the sender does not have enough balance", function () {
            ...
            describe("when the spender had an approved amount", function () {
                ...
                it("approves the requested amount and replaces the previous one", async function () {
                    ...
                    expect((await this.token.allowance(owner, spender).call()).remaining).to.be.bignumber.equal(amount);
                });
            });
        });
    });
